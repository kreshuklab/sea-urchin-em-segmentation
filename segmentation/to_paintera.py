...
def to_paintera(out_path, target, max_jobs):
    ...
    convert_to_paintera_format(
        ...
        max_threads=12, convert_to_label_multisets=True,
        restrict_sets=[-1, -1, 5, 3, 2], label_block_mapping_compression="gzip"
    )


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("-o", "--out_path", default="./tmp_paintera/data.n5")
    parser.add_argument("-t", "--target", default="local")
    parser.add_argument("-m", "--max_jobs", default=24)
    args = parser.parse_args()
    to_paintera(args.out_path, args.target, args.max_jobs)
